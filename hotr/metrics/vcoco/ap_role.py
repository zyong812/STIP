...
class APRole(object):
    ...
    def add_data(self, h_box, o_box, score, i_box, i_act, p_box, p_act):
        ...
        valid_i_inds = (i_act[:, 0] != -1) # (n_i, ) # i_act 只是一种过滤的作用
        ...
        if n_o == 0:
            ...
            # 对于每一个 human instance，每一种 action 取一个分数最高的 pair。比如 human1+hold+保留一个obj
            ...
            # 怎么对图中的 human-relation-obj 标注为 match (TP) 还是不match (FP)
            for label in range(len(self.act_name)):
                ...
                if p_inds.sum() == 0: # 没有此类关系标注时，全部认为 FP
                    self.tp[label] = np.append(self.tp[label], np.array([0]*n_h))
                    self.fp[label] = np.append(self.fp[label], np.array([1]*n_h))
                    continue
                ...
                h_over_thresh = h_over_thresh & np.expand_dims(p_inds, axis=0) # (n_h, n_p) # find only for current act -- 匹配了主语和谓语

                h_valid = h_over_thresh.sum(axis=1)>0 # (n_h, ) # at least one is True
                # h_valid -> if all is False, then argmax becomes 0. <- prevent
                assigned_p = np.argmax(h_over_thresh, axis=1) # (n_h, ) # p only for current act -- 根据主语和谓语已经找到了对应哪个 GT relation

                o_mapping_box = o_box[s_obj_argmax[label]][h_inds] # (n_h, ) # find where T is.
                p_mapping_box = p_box[assigned_p, 4:8] # (n_h, 4)

                o_overlaps = compute_overlap(o_mapping_box, p_mapping_box) # 再看宾语是不是能匹配上
                o_overlaps = np.diag(o_overlaps) # (n_h, )
                o_overlaps.setflags(write=1)
                if (~h_valid).sum() > 0:
                    o_overlaps[~h_valid] = 0 # (n_h, )

                no_role_inds = (p_box[assigned_p, 4] == -1) # (n_h, ) -- GT relation obj box 为空
                nan_box_inds = np.all(o_mapping_box == 0, axis=1) | np.all(np.isnan(o_mapping_box), axis=1) ## -- predicted relation obj box 为 0 or nan
                ...
                if self.scenario_flag: # scenario_1: 对于 GT relation obj box 为空 的情况，预测的 obj box 也要准确
                    o_iou_inds[no_role_inds & nan_box_inds] = 1
                    o_iou_inds[no_role_inds & ~nan_box_inds] = 0
                else: # scenario_2: 对于 GT relation obj box 为空 的情况，不考虑预测的 obj box，都认为是正确
                    o_iou_inds[no_role_inds] = 1

                iou_inds = (h_iou_inds & o_iou_inds)
                p_nonzero = iou_inds.nonzero()[0]
                p_inds = assigned_p[p_nonzero]
                p_iou = np.unique(p_inds, return_index=True)[1] # 去重，一个GT最多匹配一次
                p_tp = p_nonzero[p_iou]

                t = np.zeros(n_h, dtype=np.uint8)
                t[p_tp] = 1 # 最终匹配了的设为 TP
                f = 1-t # 否则设为 FP
    ...
    def evaluate(self, print_log=False):
        ...
        for label in range(len(self.act_name)):
            ...
            self.fp[label] = self.fp[label][indices] # 所有图片的 predicted relations 一起排序计算 AP
